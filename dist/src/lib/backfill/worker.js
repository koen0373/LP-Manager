...
exports.backfillPositions = backfillPositions;
const p_limit_1 = __importDefault(require("p-limit"));
const cursor_1 = require("./cursor");
const fetcher_1 = require("./fetcher");
const persist_1 = require("./persist");
const syncPositionLedger_1 = require("../sync/syncPositionLedger");
const DEFAULT_CONCURRENCY = 6;
const POSITIONS_ADDRESS = '0xD9770b1C7A6ccd33C75b5bcB1c0078f46bE46657';
/**
 * Backfill a single position
 */
async function backfillSinglePosition(tokenId, mode = 'since', sinceBlock) {
    const startTime = Date.now();
    console.log(`[BACKFILL:${tokenId}] Starting backfill (mode: ${mode})`);
    try {
        // Get or initialize cursor
        const cursor = await (0, cursor_1.getOrInitCursor)(tokenId);
        ...
        await (0, syncPositionLedger_1.syncPositionLedger)(String(tokenId));
        ...
        let totalSkipped = 0;
        while (hasMore) {
            const transfers = await (0, fetcher_1.fetchPositionTransfers)(tokenId, POSITIONS_ADDRESS, page, 100);
            if (transfers.length === 0) {
                hasMore = false;
                break;
            }
            // Convert to persist format
            const transfersToPersist = transfers.map(t => ({
            ...
            // Persist transfers
            const result = await (0, persist_1.persistTransfers)(transfersToPersist);
            totalInserted += result.inserted;
            totalUpdated += result.updated;
            totalSkipped += result.skipped;
            console.log(`[BACKFILL:${tokenId}] Page ${page}: ${result.inserted} inserted, ${result.updated} updated, ${result.skipped} skipped`);
        ...
        const currentBlock = await (0, fetcher_1.getCurrentBlockNumber)();
        await (0, cursor_1.updateCursor)(tokenId, currentBlock);
        const elapsedMs = Date.now() - startTime;
        console.log(`[BACKFILL:${tokenId}] ✅ Complete in ${elapsedMs}ms: ${totalInserted} inserted, ${totalUpdated} updated, ${totalSkipped} skipped`);
        return {
            tokenId,
            success: true,
 ...
 */
async function backfillPositions(options = {}) {
    const { tokenIds = [], mode = 'since', sinceBlock, concurrency = DEFAULT_CONCURRENCY, } = options;
    ...
    const results = await Promise.all(tokenIds.map(tokenId => limit(() => backfillSinglePosition(tokenId, mode, sinceBlock))));
    // Aggregate results

 ...
 */
async function buildPriceHistory(poolAddress, decimals0, decimals1, options = {}) {
    const { fromTimestamp = 0, toTimestamp = Math.floor(Date.now() / 1000), maxPoints = 500, } = options;
    try {
        ...
        for (const event of events) {
            ...
            // Fallback to calculating from tick
                ...
                try {
                    price = (0, poolHelpers_1.tickToPrice)(event.tick, decimals0, decimals1);
                }
                catch (error) {
 ...
 */
function downsamplePricePoints(points, targetCount) {
 ...
 */
async function buildPriceHistoryFromSwaps(poolAddress, decimals0, decimals1, options = {}) {
    const { fromTimestamp = 0, toTimestamp = Math.floor(Date.now() / 1000), maxPoints = 500, } = options;
    try {
        // Fetch Swap events from PoolEvent table
        const swaps = await prisma_1.db.poolEvent.findMany({
            where: {
                pool: poolAddress.toLowerCase(),
                eventName: 'Swap',
                timestamp: {
        ...
        for (const swap of swaps) {
            if (swap.tick === null)
                continue;
            try {
                const price = (0, poolHelpers_1.tickToPrice)(swap.tick, decimals0, decimals1);
                pricePoints.push({
 ...
 */
async function getCurrentPriceFromEvents(poolAddress, decimals0, decimals1) {
    try {
        // Try PositionEvent first
        const recentEvent = await prisma_1.db.positionEvent.findFirst({
            where: {
                pool: poolAddress.toLowerCase(),
                OR: [
                    { tick: { not: null } },
        ...
        if (recentEvent) {
            ...
            if (recentEvent.tick !== null) {
                try {
                    return (0, poolHelpers_1.tickToPrice)(recentEvent.tick, decimals0, decimals1);
                }
                catch {
                    // Fall through
                }
        ...
        const recentSwap = await prisma_1.db.poolEvent.findFirst({
            where: {
                pool: poolAddress.toLowerCase(),
                eventName: 'Swap',
                tick: { not: null },
        ...
        if (recentSwap && recentSwap.tick !== null) {
            try {
                return (0, poolHelpers_1.tickToPrice)(recentSwap.tick, decimals0, decimals1);
            }
            catch {
                return null;
            }

...
exports.getContractCreationDate = getContractCreationDate;
exports.getPositionTransferHistory = getPositionTransferHistory;
exports.getWalletPositions = getWalletPositions;
exports.getWalletPositionsViaFlareScan = getWalletPositionsViaFlareScan;
exports.getPositionNftTransfers = getPositionNftTransfers;
exports.getPositionManagerEvents = getPositionManagerEvents;
exports.getPositionCollects = getPositionCollects;
exports.getContractCodeAbi = getContractCodeAbi;
const rflrRewards_1 = require("./rflrRewards");
const tokenPrices_1 = require("./tokenPrices");
const flarescan_1 = require("../lib/adapters/flarescan");
const blockscout_1 = require("../lib/adapters/blockscout");
const rpcLogs_1 = require("../lib/adapters/rpcLogs");
const timedFetch_1 = require("../lib/util/timedFetch");
...
const FLARE_RPC_ENDPOINT = process.env.NEXT_PUBLIC_RPC_URL || 'https://flare.flr.finance/ext/bc/C/rpc';
...
const PM_INCREASE_TOPIC = '0x3067048beee31b25b2f1681f88dac838c8bba36af25bfb2b7cf7473a5847e35f';
const PM_DECREASE_TOPIC = '0x26f6a048ee9138f2c0ce266f322cb99228e8d619ae2bff30c67f8dcf9d2377b4';
const PM_COLLECT_TOPIC = '0x4d8babf9b22e68d8f3c8653392a91073d3f3d246ad70593d8c8ed3fe381b3c96';
const toTokenTopic = (tokenId) => `0x${BigInt(tokenId).toString(16).padStart(64, '0')}`;
function parseAmount(data, index) {
...
async function getContractCreationDate(contractAddress) {
    return (0, memo_1.memoize)(`contract-creation-${contractAddress}`, async () => {
        try {
            // Try Blockscout REST API first
            const response = await (0, withTimeout_1.withTimeout)((0, timedFetch_1.timedFetch)(`https://flare-explorer.flare.network/api/v2/smart-contracts/${contractAddress}`), 15000, `Contract creation fetch for ${contractAddress} timed out`);
            if (response.ok) {
                const data = await response.json();
                if (data.creation_transaction_hash) {
                    const txResponse = await (0, withTimeout_1.withTimeout)((0, timedFetch_1.timedFetch)(`https://flare-explorer.flare.network/api/v2/transactions/${data.creation_transaction_hash}`), 15000, `Transaction fetch for ${data.creation_transaction_hash} timed out`);
                    if (txResponse.ok) {
        ...
        try {
            const creation = await (0, flarescan_1.getContractCreation)(contractAddress);
            if (!creation?.txHash) {
                return null;
            }
            const receipt = (await callFlareScan('eth_getTransactionReceipt', [creation.txHash]));
            if (!receipt?.blockNumber) {
                return null;
            }
            const block = (await callFlareScan('eth_getBlockByNumber', [receipt.blockNumber, false]));
            if (!block?.timestamp) {
                return null;
            }
...
async function getPositionTransferHistory(tokenId) {
    return (0, flarescan_1.getNftTransfers)(ENOSYS_POSITION_MANAGER, tokenId);
}
// Token metadata cache
const tokenCache = new Map();
const positionsCache = new Map();
const CACHE_TTL_MS = 30000; // 30 seconds cache to avoid spamming the API
...
async function callFlareScan(method, params = [], retries = 3) {
    let attempt = 0;
    let lastError = null;
    while (attempt < retries) {
        attempt += 1;
        try {
            const response = await (0, withTimeout_1.withTimeout)((0, timedFetch_1.timedFetch)(FLARE_RPC_ENDPOINT, {
                method: 'POST',
                headers: {
            ...
            }), 15000, `RPC call ${method} timed out`);
            if (!response.ok) {
            ...
            console.warn(`[RPC] ${method} failed (attempt ${attempt}/${retries}). Retrying in ${waitTime}ms`, error);
...
async function getWalletPositions(walletAddress, options) {
    const normalizedAddress = (0, poolHelpers_1.normalizeAddress)(walletAddress);
    const cacheKey = `wallet-positions-${normalizedAddress}`;
    if (options?.refresh) {
        (0, memo_1.clearCache)(cacheKey);
        (0, memo_1.clearCache)(`viem-positions-${normalizedAddress}`);
    }
    return (0, memo_1.memoize)(cacheKey, async () => {
        try {
            ...
            try {
                const response = await (0, withTimeout_1.withTimeout)((0, timedFetch_1.timedFetch)(`/api/positions?address=${normalizedAddress}${options?.refresh ? '&refresh=1' : ''}`, {
                    method: 'GET',
                    headers: {
                        Accept: 'application/json',
                    },
                }), 15000, `Positions API call for ${normalizedAddress} timed out`);
                if (response.ok) {
                ...
                console.warn(`Positions API response not OK (${response.status}). Falling back to Viem/FlareScan directly.`, errorText);
            }
            catch (apiError) {
...
async function getWalletPositionsViaFlareScan(walletAddress) {
    try {
        const normalised = (0, poolHelpers_1.normalizeAddress)(walletAddress);
        const factory = await (0, poolHelpers_1.getFactoryAddress)(ENOSYS_POSITION_MANAGER);
        ...
        const processed = await (0, concurrency_1.mapWithConcurrency)(indices, 12, async (i) => processPosition(normalised, i, factory));
        return processed
...
async function processPosition(walletAddress, index, factory) {
    try {
        ...
        // Parse position data
        return await parsePositionData(tokenId, positionData, factory, walletAddress);
    }
    catch (error) {
...
async function parsePositionData(tokenId, positionData, factory, walletAddress) {
    try {
        ...
        const decoded = (0, poolHelpers_1.decodePositionData)(positionData);
        const token0 = (0, poolHelpers_1.normalizeAddress)(decoded.token0);
        const token1 = (0, poolHelpers_1.normalizeAddress)(decoded.token1);
        const fee = Number(decoded.fee);
        const tickLower = (0, poolHelpers_1.toSignedInt24)(decoded.tickLower);
        const tickUpper = (0, poolHelpers_1.toSignedInt24)(decoded.tickUpper);
...
async function getPositionNftTransfers(positionsAddress, tokenId, options) {
    ...
    return (0, memo_1.memoize)(cacheKey, async () => {
        try {
            // Try Blockscout REST API first
            const response = await (0, withTimeout_1.withTimeout)((0, timedFetch_1.timedFetch)(`https://flare-explorer.flare.network/api/v2/tokens/${positionsAddress}/instances/${tokenId}/transfers?page=1&page_size=100`), 15000, `NFT transfers fetch for token ${tokenId} timed out`);
            if (response.ok) {
                const data = await response.json();
        ...
        // Fallback to Flarescan API
        const transfers = await (0, flarescan_1.getNftTransfers)(positionsAddress, tokenId);
...
async function getPositionManagerEvents(tokenId, fromBlock, toBlock, options) {
    ...
    return (0, memo_1.memoize)(cacheKey, async () => {
        ...
        const fetchLogs = async (topic) => {
            try {
                const logs = await (0, blockscout_1.fetchLogsViaBlockscout)({
                    address,
                    fromBlock,
                    toBlock,
                ...
                try {
                    const logs = await (0, rpcLogs_1.fetchLogsViaRpc)({
                        address,
                        fromBlock,
                        toBlock,
                        topics: [topic, tokenTopic],
        ...
        return [...increaseEvents, ...decreaseEvents, ...collectEvents].sort((a, b) => a.blockNumber === b.blockNumber ? a.txHash.localeCompare(b.txHash) : a.blockNumber - b.blockNumber);
    }, 60000);
}
// Function to get collect events for a specific position (Blockscout REST v2 API)
async function getPositionCollects(poolAddress, positionsAddress, tokenId) {
    try {
        const transfers = await getPositionNftTransfers(positionsAddress, tokenId);
        const mintTransfer = transfers.find((transfer) => transfer.from === '0x0000000000000000000000000000000000000000');
        const fromBlock = mintTransfer?.blockNumber ?? 0;
        const toBlock = await (0, rpcLogs_1.fetchLatestBlockNumber)();
        let logs = await (0, blockscout_1.fetchLogsViaBlockscout)({
            address: poolAddress,
            fromBlock,
            toBlock,
        ...
        if (logs.length === 0) {
            logs = await (0, rpcLogs_1.fetchLogsViaRpc)({
                address: poolAddress,
                fromBlock,
                toBlock,
                topics: [COLLECT_TOPIC],
...
async function getContractCodeAbi(contractAddress) {
    try {
        const sourceEntry = await (0, flarescan_1.getContractSourceCode)(contractAddress);
        if (!sourceEntry) {

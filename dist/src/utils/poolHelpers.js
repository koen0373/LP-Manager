...
exports.toSignedInt24 = toSignedInt24;
...
exports.decodePositionData = decodePositionData;
exports.tickToSqrtRatio = tickToSqrtRatio;
exports.sqrtRatioToPrice = sqrtRatioToPrice;
exports.tickToPrice = tickToPrice;
exports.calcAmountsForPosition = calcAmountsForPosition;
exports.isInRange = isInRange;
exports.calculateAccruedFees = calculateAccruedFees;
exports.isStableSymbol = isStableSymbol;
exports.calculatePositionValue = calculatePositionValue;
exports.formatPrice = formatPrice;
exports.computePriceRange = computePriceRange;
const viem_1 = require("viem");
const chainFlare_1 = require("../lib/chainFlare");
function normalizeAddress(address) {
    ...
    return (0, viem_1.getAddress)(normalized);
}
...
function invalidateCache(poolAddress, factoryAddress) {
    if (poolAddress) {
...
const publicClient = (0, viem_1.createPublicClient)({
    ...
    transport: (0, viem_1.fallback)([
        (0, viem_1.http)(chainFlare_1.flare.rpcUrls.default.http[0], { batch: true }),
        (0, viem_1.http)('https://flare.flr.finance/ext/bc/C/rpc', { batch: true }),
    ]),
    pollingInterval: 6000,
});
const DEFAULT_FACTORY_ADDRESS = '0x17AA157AC8C54034381b840Cb8f6bf7Fc355f0de';
// PositionManager ABI
...
function bigIntToDecimal(amount, decimals) {
    if (amount === BigInt(0))
        return 0;
...
async function getFactoryAddress(positionManager) {
    ...
    try {
        ...
        const factory = await Promise.race([
            ...
            new Promise((_, reject) => setTimeout(() => reject(new Error('RPC call timeout')), 10000))
        ]);
        ...
        console.warn(`[RPC] Falling back to static factory address ${fallbackFactory}. Ensure RPC connectivity is restored.`);
...
async function getPoolState(poolAddress) {
    ...
    try {
        ...
        const [sqrtPriceX96, tickRaw] = slot0;
...
function subtractUint256(a, b) {
    if (a >= b) {
        return a - b;
    }
    return UINT256_MOD - (b - a);
}
async function getFeeGrowthGlobals(poolAddress) {
    const cacheKey = poolAddress.toLowerCase();
...
async function getTickData(poolAddress, tick) {
...
async function getFeeGrowthInside(poolAddress, tickLower, tickUpper, currentTick) {
    ...
    const feeGrowthInside0X128 = subtractUint256(subtractUint256(globals.feeGrowthGlobal0X128, feeGrowthBelow0), feeGrowthAbove0);
    const feeGrowthInside1X128 = subtractUint256(subtractUint256(globals.feeGrowthGlobal1X128, feeGrowthBelow1), feeGrowthAbove1);
    return {
...
function decodePositionData(positionData) {
    const decoded = (0, viem_1.decodeFunctionResult)({
        abi: POSITION_MANAGER_ABI,
        functionName: 'positions',
        data: positionData,
    });
    const [nonce, operator, token0, token1, fee, tickLower, tickUpper, liquidity, feeGrowthInside0LastX128, feeGrowthInside1LastX128, tokensOwed0, tokensOwed1] = decoded;
    return {
        nonce,
        operator,
...
function sqrtRatioToPrice(sqrtRatioX96, token0Decimals, token1Decimals) {
...
function calcAmountsForPosition(liquidity, sqrtPriceX96, tickLower, tickUpper, token0Decimals, token1Decimals) {
...
async function calculateAccruedFees(params) {
    const { poolAddress, liquidity, tickLower, tickUpper, currentTick, feeGrowthInside0LastX128, feeGrowthInside1LastX128, tokensOwed0, tokensOwed1, } = params;
    const { feeGrowthInside0X128, feeGrowthInside1X128 } = await getFeeGrowthInside(poolAddress, tickLower, tickUpper, currentTick);
...
async function calculatePositionValue(params) {
    const { token0Symbol, token1Symbol, token0Decimals, token1Decimals, amount0Wei, amount1Wei, fee0Wei, fee1Wei, sqrtPriceX96, } = params;
...
function computePriceRange(tickLower, tickUpper, token0Decimals, token1Decimals) {

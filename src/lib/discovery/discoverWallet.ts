...
import { findPositionsByWallet } from './findPositions';
import { enrichPositions } from './enrichPosition';
import type { DiscoveryResult, DiscoveryOptions, EnrichedPosition } from './types';
...
export async function discoverWalletPositions(
  ...
  try {
    ...
    const tokenIds = await findPositionsByWallet(walletAddress, {
    ...
    const enrichedPositions = await enrichPositions(tokenIds, walletAddress);
    ...
    if (options.minTvlUsd !== undefined) {
      filteredPositions = filteredPositions.filter(
        (pos) => pos.tvlUsd >= (options.minTvlUsd || 0)
      );
    }
    ...
    const totalRewardsUsd = enrichedPositions.reduce(
      (sum, pos) => sum + pos.rflrUsd,
      0
    );

    const duration = Date.now() - startTime;
    console.log(
      `[DISCOVERY] Completed wallet discovery for ${walletAddress} in ${duration}ms: ${filteredPositions.length} positions (${activePositions.length} active, ${inactivePositions.length} inactive)`
    );
 ...
 */
export function groupPositionsByStatus(positions: EnrichedPosition[]): {
  active: EnrichedPosition[];
  inactive: EnrichedPosition[];
} {
  const active: EnrichedPosition[] = [];
  const inactive: EnrichedPosition[] = [];
 ...
 */
export function sortPositionsByTvl(
 ...
 */
export function sortPositionsByRewards(
  positions: EnrichedPosition[]
): EnrichedPosition[] {

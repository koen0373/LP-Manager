...
export async function getCollectEventsForToken(tokenId: string, poolAddress?: string): Promise<CollectEvent[]> {
  return memoize(`collect-events-${tokenId}`, async () => {
...
async function fetchCollectEventsChunk(
  fromBlock: number,
  toBlock: number,
  tokenIdHex: string,
  contractAddress: string
): Promise<CollectEvent[]> {
  try {
    ...
    return data.result.map((log: { data: string; blockNumber: string; transactionHash: string; timeStamp: string; topics: string[] }) => {
...
export function clearCollectEventsCache(tokenId?: string): void {
  if (tokenId) {
    clearCache(`collect-events-${tokenId}`);
  } else {
    clearCache();
  }
}
